...
class Food_Recipe_Blog_Button extends WP_Customize_Section {
	...
	public function json() {
		...
		if ( ! $this->button_url && $food_recipe_blog_theme->get( 'ThemeURI' ) ) {

			$button_url = $food_recipe_blog_theme->get( 'ThemeURI' );

		// Fall back to the `Author URI` defined in `style.css`.
		} elseif ( ! $this->button_url && $food_recipe_blog_theme->get( 'AuthorURI' ) ) {

			$button_url = $food_recipe_blog_theme->get( 'AuthorURI' );
		}

		$json['button_text'] = $this->button_text ? $this->button_text : $food_recipe_blog_theme->get( 'Name' );
